...
# TODO: ADDED BY MOHAMMAD KACHUEE
DEBUG = False
...
class TdtIO(BaseIO):
                ...
                for channel in channels:
                    ...
                    if type_label in ['EVTYPE_STRON', 'EVTYPE_STROFF']:
                        ...
                        if lazy:
                            ...
                            # FIXME: MODEFIED BY MOHAMMAD KACHUEE
                            if DEBUG:
                                print(sev_filename)
                            try:
    ...
    def read_block(self, lazy=False, cascade=True, blockname=''):
        bl = Block()
        tankname = os.path.basename(self.dirname)
    ...
    def read_block_improved(self, blockname=''):
        """
        Improved version by Mohammad Kachuee
        """
        # read tev files
        bl = Block()
        tankname = os.path.basename(self.dirname)
        ...
        # read sev files
        sev_data = {'sev':octave.SEV2mat_v2(self.dirname+'/'+blockname)['Cont']['data']}

        # merge sev and tev data
        block_data.update(sev_data)
